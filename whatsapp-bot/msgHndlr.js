...
const { between, Chordlagu, liriklagu, quotemaker, randomNimek, fb, sleep, jadwalTv } = require('./lib/functions')
const brainlyv2 = require('./lib/brainly')
const { help, def, snk, info, donate, readme, listChannel } = require('./lib/help')
	...
	if (!isGroupMsg) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mMSG\x1b[1;37m]', time, color(msgs(command)), 'from', color(pushname))
        ...
        case '!stiker':
            if (isMedia && type === 'image') {
                ...
                console.log(mimetype, mediaData)
		await client.sendImageAsSticker(from, imageBase64)
            ...
            break
	case '!brainly':
		if (args.length === 1) return client.reply(from, `Masukan perintah *!brainly [Jumlah Pertanyaan] [pertanyaanmu]* / *!brainly [pertanyaanmu]*\ncontoh : *!brainly 2 Apa itu pancasila* / *!brainly apa itu pancasila**`, id)
		if (args.length >= 2){
                let tanya = args.slice(2, args.length+1)
                let jum = args[1]
		await brainlyv2(tanya,Number(jum), function(res){
		console.log('res :',res)
		console.log('hs :',hs)
	...
	case '!pasangan':
		...
		var namamu = gh.split("&")[0];
                ...
                request.get({
                        ...
                        url:     'http://www.primbon.com/kecocokan_nama_pasangan.php?nama1='+ namamu +'&nama2='+ pasangan
	...
	case '!stim':
        ...
        case '!ig':
            ...
            try {
             ...
             //	await client.reply(from, 'Sayangnya server untuk download ig sudah tidak beroperasi, kami sedang mengusahakan pengembangan untuk fitur ini terima kasih.', id)
		await client.sendFileFromUrl(from, resp.result, `Insta${ext}`, '', id)
            } catch {
        ...
        case '!infogempa':
            ...
            const { waktu, potensi, kedalaman, lokasi, magnitude, map, koordinat } = bmkg
            const hasil = `INFORMASI GEMPA *BMKG*\n\n➸ *Waktu* : ${waktu}\n➸ *Kedalaman* : ${kedalaman}\n➸ *Lokasi* : ${lokasi}\n➸ *Koordinat* : ${koordinat}\n➸ *Magnitude* : ${magnitude}\n➸ *Potensi* : ${potensi}`
            //await client.reply(from, hasil, id)
            await client.sendFileFromUrl(from, map, 'Map-Gempa.jpg', hasil, id)
	    break
        case '!nh':
            if (!isOwner) return
...
//console.log(total_pages, results[0].width, results[1].height, results[3].alt_description, results[4].urls.full)
	...
	case '!quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    client.sendFile(from, amsu, 'quotesmaker.jpg','Nih buat kamu >//<').then(cih => console.log(cih)).catch(err => {
   ...
   /*         if (!isGroupMsg) return await client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
     ...
     */       await client.reply(from, `Fitur kudeta dimatikan hehe\n*[req by mumut]*`, id)
            break
        case '!leaveall':
            if (!isOwner) return await client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
        ...
        case '!leave':
            if (!isGroupMsg) return await client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return await client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            await client.sendText(from,'Byebyee semua:)').then(() => client.leaveGroup(groupId))
            break
        case '!promote':
            if (!isGroupMsg) return await client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
        ...
        case '!getses':
            ...
            client.sendFile(from, sesPic, 'session.png', 'Nih buat kamu >//<', id)
            break
        case '!chord':
            if (args.length == 1) return await client.reply(from, 'Kirim perintah *!Chord [optional]*, contoh *!chord aku bukan boneka*', id)
            const clagu = body.slice(7)
            console.log(clagu)
            const chord = await Chordlagu(clagu)
            await client.reply(from, chord, id)
            break
	case '!lirik':
	...
	case '!menu':
        ...
        case '!snk':
            ...
            break
	default:
	// await client.reply(from, def, id)
    	}
      } catch (err) {
