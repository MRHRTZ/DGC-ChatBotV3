...
const { liriklagu, quotemaker, randomNimek, fb, ig, twt, sleep, jadwalTv, ss, between } = require('./lib/functions')
...
module.exports = msgHandler = async (client, message) => {
    try {
           ...
           // if (args.length === 1 && !isMedia || args.length === 1 && !quotedMsg) return client.reply(from, `Kirim foto dengan caption *!stickernobg*`, id)
            if (isMedia && type === 'image') {
              try {
                ...
                var result = await removeBackgroundFromImageBase64({ base64img, apiKey: '5tHjBbPSh3gYoHD9rTqiVCa7', size: 'auto', type: 'auto', outFile })
        ...
        case '!donasi':
        case '!donate':
            client.sendLinkWithAutoPreview(from, 'https://saweria.co/MRHRTZ', donate)
            break
        case '!tts':
            try {
        ...
        case '!nulis':
            ...
            .then(res => res.json())
            .then((json) => {
                client.sendFile(from, json.result, 'tulisan.jpg', `Udah di tulis ni ${pushname}`, id);
        ...
        case '!ytmp3':
            ...
            try {
                ...
                const respReal = await get.get('http://scrap.terhambar.com/yt?link='+ args[1]).json()
                if (resp.error) {
                    client.reply(from, resp.error, id)
                } else {
                    const { title, thumb, filesize, result } = await resp
                    const { linkAudioOnly } = await respReal
                    ...
                    await client.sendFileFromUrl(from, linkAudioOnly, `${title}.mp3`, '', id).catch(() => client.reply(from, mess.error.Yt3, id))
                }
            } catch (err) {
        ...
        case '!ytmp4':
            ...
            try {
                ...
                const ytvReal = await get.get('http://scrap.terhambar.com/yt?link='+ args[1]).json()
                const short4 = await get.get(`https://api.haipbis.xyz/bitly?url=${ytv.result}`).json()
                if (ytv.error) {
                    await client.reply(from, ytv.error, id)
                } else {
                    const { linkVideo } = await ytvReal
                //    if (Number(ytv.filesize.split(' MB')[0]) > 40.00) return await client.reply(from, 'Maaf durasi video sudah melebihi batas maksimal!', id)
                    client.sendFileFromUrl(from, ytv.thumb, 'thumb.jpg', `➸ *Title* : ${ytv.title}\n➸ *Filesize* : ${ytv.filesize}\n➸ *Link Download* : ${short4.result}\n\n_Apabila tidak terdownload otomatis refresh page tersebut setelah membuka link download_`, id)
                 //   await client.sendFileFromUrl(from, linkVideo, `${ytv.title}.mp4`, '', id).catch(() => client.reply(from, mess.error.Yt4, id))
                }
            } catch (er) {
        ...
        case '!quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    client.sendFile(from, amsu, 'quotesmaker.jpg', `Nih buat kamu >//< ${pushname}`).catch(() => {
